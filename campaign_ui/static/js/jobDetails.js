...
// Default max weights for each factor (percentage)
...
function openRankingModal(companyName, jobTitle, score, breakdown) {
    ...
    // Handle breakdown - parse if string, use directly if object
    ...
    if (!breakdownData || typeof breakdownData !== 'object' || Object.keys(breakdownData).length === 0) {
        detailsContainer.innerHTML = '<div class="ranking-empty-state">Ranking explanation not available</div>';
    ...
    factors.forEach(([factor, value]) => {
        ...
        // Use default max as fallback, but allow actual to exceed it
    ...
    factors.forEach(([factor, value]) => {
        ...
        item.innerHTML = `
            ...
            <div class="ranking-progress-bar">
                <div class="ranking-progress-fill ${colorClass}" style="width: ${Math.min(100, Math.max(0, percentage))}%"></div>
            </div>
        `;
