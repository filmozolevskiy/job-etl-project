...
def edit_profile(profile_id):
    ...
    if request.method == 'POST':
        ...
        if errors:
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("SELECT * FROM marts.profile_preferences WHERE profile_id = %s", (profile_id,))
                profile = cur.fetchone()
...
def toggle_active(profile_id):
    ...
    try:
        with conn.cursor() as cur:
            # Get current status
            cur.execute("SELECT is_active FROM marts.profile_preferences WHERE profile_id = %s", (profile_id,))
            result = cur.fetchone()
...
def delete_profile(profile_id):
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("SELECT profile_name FROM marts.profile_preferences WHERE profile_id = %s", (profile_id,))
            result = cur.fetchone()
            ...
            # Delete profile
            cur.execute("DELETE FROM marts.profile_preferences WHERE profile_id = %s", (profile_id,))
            conn.commit()
