...
class TestExtractNormalizeRankFlow:
    ...
    def test_normalize_jobs_to_staging(
        ...
        if result.returncode != 0:
            pytest.skip(f"dbt run failed: {result.stderr}. This test requires dbt to be configured properly.")
    ...
    def test_complete_flow_end_to_end(
        ...
        # Verify raw layer
        with db.get_cursor() as cur:
            cur.execute("SELECT COUNT(*) FROM raw.jsearch_job_postings WHERE profile_id = %s", 
                       (test_profile["profile_id"],))
        ...
        with db.get_cursor() as cur:
            cur.execute("SELECT COUNT(*) FROM staging.jsearch_job_postings WHERE profile_id = %s", 
                       (test_profile["profile_id"],))
        ...
        # Verify marts layer
        with db.get_cursor() as cur:
            cur.execute("SELECT COUNT(*) FROM marts.fact_jobs WHERE profile_id = %s", 
                       (test_profile["profile_id"],))
