...
def main():
    ...
    print(f"   Jobs with skills: {stats['jobs_with_skills']} ({stats['jobs_with_skills']/max(stats['total_jobs'], 1)*100:.1f}%)")
    print(f"   Jobs with seniority: {stats['jobs_with_seniority']} ({stats['jobs_with_seniority']/max(stats['total_jobs'], 1)*100:.1f}%)")
    ...
    if single_words:
        ...
        for i, term in enumerate(single_words[:30], 1):
            print(f"  {i:2d}. {term['term']:30s} | Freq: {term['frequency']:4d} | "
    ...
    if two_words:
        ...
        for i, term in enumerate(two_words[:20], 1):
            print(f"  {i:2d}. {term['term']:40s} | Freq: {term['frequency']:4d} | "
                  f"In: {term['appears_in']:12s}")
